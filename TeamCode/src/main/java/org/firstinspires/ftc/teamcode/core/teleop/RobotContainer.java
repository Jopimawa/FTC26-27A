...
import org.firstinspires.ftc.teamcode.command.debug.DriveToggleEncoders;
import org.firstinspires.ftc.teamcode.subsystem.CellSubsystem;
import org.firstinspires.ftc.teamcode.subsystem.DriveSubsystem;
import org.firstinspires.ftc.teamcode.subsystem.FlowerSubsystem;
...
public class RobotContainer {
    ...
    public RobotContainer(HardwareMap hardwareMap, Telemetry telemetry, Gamepad gamepad1, Gamepad gamepad2) {
        ...
        flower = new FlowerSubsystem(hardwareMap, telemetry);
        cell =   new CellSubsystem(hardwareMap, telemetry);

        // gamepad[0] is the first gamepad, gamepad[1] is the second -jr
    ...
    public void configureBindings() {
        ...
        gamepad[1].getGamepadButton(GamepadKeys.Button.DPAD_UP)
                  .whenHeld(new DriveRawMove(drive,debugDrivePower,0,0,0));
        gamepad[1].getGamepadButton(GamepadKeys.Button.DPAD_RIGHT)
                .whenHeld(new DriveRawMove(drive,0,debugDrivePower,0,0));
        gamepad[1].getGamepadButton(GamepadKeys.Button.DPAD_LEFT)
                  .whenHeld(new DriveRawMove(drive,0,0,debugDrivePower,0));
        gamepad[1].getGamepadButton(GamepadKeys.Button.DPAD_DOWN)
                .whenHeld(new DriveRawMove(drive,0,0,0,debugDrivePower));

        gamepad[1].getGamepadButton(GamepadKeys.Button.X)
                .whenHeld(new DriveToggleEncoders(drive, true));
        gamepad[1].getGamepadButton(GamepadKeys.Button.Y)
